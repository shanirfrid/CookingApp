package com.example.shanir.cookingappofshanir.utils;

public class DbConstants {
    public static final String USER_TABLE_NAME = "Users";
    public static final String FAVORITE_RECIPES ="UserFavoriteRecipes";
    public static final String ADMIN_EMAIL ="shanir207@gmail.com";
    public static final String USER_INGREDIENTS_SUB_TABLE_NAME = "ingredients";
    public static final String RECIPE_FAVORITE_NAMES = "recipeNames";
    public static final String RECIPE_TABLE_NAME = "Recipes";
    public static final String USER_INGREDIENTS_TABLE_NAME = "UserIngredients";
    public static final String APP_RECIPE_IMAGES_FULL_URL = "gs://cookingappofshanir.appspot.com/images/";
    public static final String APP_PROFILE_IMAGES_FULL_URL = "gs://cookingappofshanir.appspot.com/imagesprofil/";
    public static final String PROFILE_IMAGES_URL = "imagesprofil/";
    public static final String RECIPE_IMAGES_URL = "images/";
    public static final String PROFILE_IMAGE_FILE_NAME_CAMERA = "userpicprofil";
    public static final String ADD_RECIPE_IMAGE_FILE_NAME_CAMERA = "userpic";
    public static final long ONE_MEGABYTE = 1024 * 1024 * 5;
    public static final int MINIMAL_PASSWORD_SIZE = 6;
}
